(function ($) {
  ...
  $("#install-activate-button").on("click", function (e) {
    ...
    var activationData = {
      action: "homedroid_install_and_activate_plugins",
    };
    $.post(homedroid_localize.ajax_url, activationData, function (response) {
      ...
      if (match) {
        ...
        try {
          ...
          if (responseObj.success === true) {
            window.location.href = homedroid_localize.redirect_url;
          } else {
            console.log("Error!");
          }
        } catch (error) {
